 ...
 * halter_form.php
 *
 * Formular zum Anlegen/Bearbeiten eines Halters. Portal-Prinzip: nur
 * Ersteller oder Admin dürfen einen bestehenden Halter öffnen (siehe
 * Halter::findByIdForUser()). NICHT mehr admin-only (war früher
 * admin_add_halter.php).
 */
...
$existing   = $editingId ? Halter::findByIdForUser($editingId, $currentUserId, $isAdmin) : null;
$isEditMode = $existing !== null;

if ($editingId !== null && $existing === null) {
    http_response_code(404);
    die('Der angeforderte Halter wurde nicht gefunden.');
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$imageResult['success']) {
        ...
        if ($isEditMode) {
            $result = Halter::update($editingId, $halterData, $image, $removeImage);
        } else {
            $result = Halter::create($halterData, $currentUserId, $image);
        }
    ...
    $existing = array_merge($existing ?? [], $halterData);
}

$pageTitle = $isEditMode ? 'Halter bearbeiten' : 'Halter anlegen';
...
<div class="bg-white/80 rounded-2xl shadow border border-sand p-4 sm:p-6 mt-4 max-w-xl">
    <h1 class="text-xl font-extrabold text-fellDk mb-4">
        <?= $isEditMode ? 'Halter bearbeiten' : 'Neuen Halter anlegen' ?>
    </h1>
    ...
    <form method="post" action="/halter_form.php<?= $isEditMode ? '?id=' . (int) $editingId : '' ?>"
          enctype="multipart/form-data" class="space-y-4">
        <?= csrf_field() ?>

        <div
            x-data='bildCropper({
                initialPreviewUrl: <?= $isEditMode && !empty($existing['hat_bild']) ? json_encode('/halterbild.php?id=' . (int) $editingId . '&v=' . urlencode((string) ($existing['bild_updated_at'] ?? '')), JSON_HEX_APOS) : 'null' ?>
            })'
            x-init="init()"
        >
        ...
        <div class="flex flex-col sm:flex-row gap-3 pt-2">
            <button type="submit" class="bg-tanne hover:bg-tanneDk text-creme font-bold text-sm px-6 py-2.5 rounded-full transition-colors">
                <?= $isEditMode ? 'Speichern' : 'Halter anlegen' ?>
            </button>
            <a href="/halter.php" class="text-sm text-fellDk/60 self-center hover:text-fellDk">Abbrechen</a>
        </div>
    </form>
</div>
 ...
 * bewusst 1:1 übernommen statt in eine gemeinsame Datei ausgelagert,
 * da diese App ohne Build-Step arbeitet (Tailwind/Alpine via CDN) und
 * ein zusätzliches <script src="..."> pro Formular denselben Effekt
 * hätte, nur mit einem zusätzlichen HTTP-Request.
 */
